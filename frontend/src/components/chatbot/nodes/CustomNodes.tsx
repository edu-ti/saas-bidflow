...
const NodeWrapper = ({ children, selected, title, icon: Icon, color }: any) => (
 <div className={`min-w-[200px] bg-bg-secondary border-2 rounded-xl overflow-hidden transition-all duration-300 ${
 selected ? `border-primary scale-[1.02]` : 'border-border hover:border-border'
 }`}>
 <div className={`p-3 flex items-center gap-2 border-b border-border bg-bg-tertiary/50 ${color}`}>
 <Icon size={16} className="text-primary" />
 <span className="text-xs font-semibold uppercase tracking-widest text-text-primary">{title}</span>
 </div>
 <div className="p-4 text-sm text-text-secondary leading-relaxed font-medium">
 {children}
 </div>
 </div>
);
...
export const QuestionNode = memo(({ data, selected }: NodeProps) => (
 ...
 <div key={i} className="px-2 py-1 bg-bg-tertiary rounded border border-border relative group">
 {opt}
 <Handle 
 type="source" 
...
export const CaptureNode = memo(({ data, selected }: NodeProps) => (
 ...
 <div className="px-2 py-1 bg-background/50 border border-border rounded italic text-text-muted text-xs">
...
export const ActionNode = memo(({ data, selected }: NodeProps) => (
 ...
 <Handle type="target" position={Position.Top} className="!w-3 !h-3 !bg-primary !border-2 !border-background" />
 <div className="flex items-center gap-2 text-primary font-semibold uppercase tracking-tight">
