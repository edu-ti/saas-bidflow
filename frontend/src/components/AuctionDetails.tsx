...
export default function AuctionDetails() {
 ...
 'Encerrada': 'bg-bg-tertiary/40 text-text-muted border-border/30',
 };
 return styles[status] || 'bg-bg-tertiary/40 text-text-muted border-border/30';
 };
 ...
 <p className="font-semibold uppercase text-xs text-text-muted">Buscando\.\.\.</p>
 </div>
 );
 }
 ...
 Bidding <span className="text-primary">Monitoring Central</span>
 </h1>
 ...
 <span className="text-xs font-semibold uppercase tracking-widest text-text-muted opacity-60">Sessões Ativas</span>
 <span className="text-sm font-semibold text-primary tracking-tight">{biddingsList.length}</span>
 </div>
 ...
 <tr>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60">Identificação Estratégica</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60">Órgão Licitante</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60 text-center">Cronograma Disputa</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60 text-center">Status Operacional</th>
 ...
 <tr key={bid.id} className="hover:bg-bg-tertiary/20 transition-all group border-b border-border/10 duration-500">
 <td className="px-10 py-10">
 <div className="font-semibold text-text-primary group-hover:text-primary transition-colors uppercase tracking-tight text-sm">{bid.bidding_metadata?.edital || 'SEM EDITAL'}</div>
 <div className="text-xs text-text-muted font-mono tracking-widest mt-1 opacity-60 uppercase">Process: {bid.process_number || 'N/A'}</div>
 </td>
 ...
 <span className="text-xs text-primary font-semibold uppercase tracking-widest bg-primary/5 px-2 py-0.5 rounded border border-primary/20">{bid.modality}</span>
 </div>
 </td>
 <td className="px-10 py-10 text-center">
 <div className="text-text-primary font-semibold text-xs uppercase tracking-widest">{formatDate(bid.opening_date)}</div>
 <div className="text-xs text-text-muted font-semibold uppercase tracking-widest mt-1 opacity-40">{formatTime(bid.opening_date)}</div>
 </td>
 <td className="px-10 py-10 text-center">
  <span className={`px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wider rounded-xl border whitespace-nowrap ${getStatusBadge(bid.status)}`}>
 ...
 className="flex items-center gap-3 text-text-muted hover:text-primary transition-all text-xs font-semibold uppercase group"
 >
 <div className="p-2 bg-bg-tertiary/40 border border-border rounded-lg group-hover:bg-primary group-hover:text-white transition-all ">
 <ArrowLeft size={14} />
 </div>
 Retornar à Matriz
 </button>
 <div className="space-y-2">
 <h1 className="text-4xl font-semibold tracking-tight text-text-primary uppercase leading-tight max-w-4xl">{bidding.title}</h1>
 <div className="flex items-center gap-6 text-xs font-semibold uppercase text-text-muted opacity-60">
 ...
 <button className="flex items-center gap-4 px-8 py-4 bg-bg-tertiary/40 text-text-primary font-semibold rounded-2xl border border-border/30 hover:bg-bg-tertiary transition-all text-xs uppercase tracking-widest ">
 ...
 <div className="card p-10 space-y-10 bg-bg-tertiary/10 backdrop-blur-xl border-border/30 ">
 <div className="flex items-center gap-4 border-b border-border/30 pb-6">
 <Info size={24} className="text-primary" />
 <h2 className="text-sm font-semibold uppercase text-text-primary">Engenharia do Edital</h2>
 </div>
 
 <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
 <div className="space-y-3">
 <span className="text-xs font-semibold text-text-muted uppercase opacity-60">Órgão Comprador</span>
 <p className="text-xs font-semibold text-text-primary uppercase leading-relaxed tracking-tight">{bidding.agency}</p>
 </div>
 <div className="space-y-3">
 ...
 <div>
 <span className="text-xs font-semibold text-text-muted uppercase block opacity-60">Sessão Pública</span>
 <p className="text-sm font-semibold text-text-primary uppercase tracking-widest">{formatDate(bidding.opening_date)}</p>
 </div>
 </div>
 <div className="flex items-center gap-5">
 <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center text-primary border border-primary/20 group hover:scale-110 transition-transform">
 ...
 <div>
 <span className="text-xs font-semibold text-text-muted uppercase block opacity-60">Cronologia Core</span>
 <p className="text-sm font-semibold text-text-primary uppercase tracking-widest">{formatTime(bidding.opening_date)}</p>
 </div>
 </div>
 <div className="flex items-center gap-5">
 <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center text-primary border border-primary/20 group hover:scale-110 transition-transform">
 ...
 <span className="text-xs font-semibold text-text-muted uppercase block opacity-60">Portal de Compra</span>
 <a href={`https://${bidding.source_url}`} target="_blank" rel="noreferrer" className="text-sm font-semibold text-primary truncate block hover:text-primary-hover transition-colors">{bidding.source_url?.toUpperCase()}</a>
 </div>
 </div>
 </div>

 <div className="space-y-5 pt-6">
 <span className="text-xs font-semibold text-text-muted uppercase flex items-center gap-4 opacity-60"><AlignLeft size={16} /> Objeto de Contratação Estratégica</span>
 <p className="text-sm text-text-secondary leading-relaxed italic bg-bg-tertiary/20 p-8 rounded-xl border border-border/30 uppercase tracking-tight">{bidding.description}</p>
 </div>
 </div>

 {/* Supplier Grid */}
 <div className="space-y-10">
 <h3 className="text-xs font-semibold uppercase text-text-primary flex items-center gap-5 px-4">
 ...
 <div key={idx} className="card overflow-hidden bg-bg-tertiary/10 backdrop-blur-md border-border/30 group hover:border-primary/30 transition-all duration-700">
 <div className="bg-bg-tertiary/30 px-10 py-6 flex justify-between items-center border-b border-border/30">
 <div className="flex items-center gap-5">
 <div className="w-3 h-3 rounded-full bg-primary animate-pulse"></div>
 <span className="text-sm font-semibold uppercase tracking-widest text-text-primary group-hover:text-primary transition-colors">{supplier.name}</span>
 </div>
 <span className="text-xs font-semibold uppercase text-primary italic bg-primary/5 px-4 py-1.5 rounded-full border border-primary/20 ">{supplier.status}</span>
 </div>
 <div className="p-10 overflow-x-auto ">
 <table className="w-full text-left">
 <thead className="text-xs font-semibold uppercase text-text-muted opacity-60 border-b border-border/30">
 <tr>
 ...
 <div className="text-xs font-semibold text-text-primary uppercase tracking-tight">Item 21: DEA - Desfibrilador Externo Automático</div>
 <div className="text-xs text-text-muted font-semibold uppercase tracking-widest mt-2 opacity-60 flex items-center gap-2"><Target size={12} className="text-primary/40" /> Qtd Demandada: 02 UN</div>
 </td>
 <td className="py-8 text-sm font-mono text-text-secondary uppercase tracking-widest font-semibold opacity-80">{supplier.fab}</td>
 <td className="py-8 text-right font-semibold text-text-primary text-base tracking-tight group-hover:text-primary transition-colors">R$ {supplier.valUnit}</td>
 <td className="py-8 text-right">
 <button className="p-3 bg-bg-tertiary/40 border border-border rounded-xl text-text-muted hover:text-primary transition-all group-hover:scale-110"><Eye size={18} /></button>
 </td>
 </tr>
 </tbody>
 </table>
 ...
 <div className="space-y-10">
 {/* Document widget */}
 <div className="card p-8 space-y-8 bg-bg-tertiary/10 backdrop-blur-md border-border/30 ">
 <h3 className="text-xs font-semibold uppercase text-text-primary flex items-center gap-3">
 <FileCheck size={16} className="text-primary" /> Repositório Documental
 </h3>
 <div className="space-y-6">
 <div className="flex items-center justify-center border-2 border-dashed border-border rounded-xl p-12 hover:border-primary/40 hover:bg-primary/5 transition-all group cursor-pointer duration-500 ">
 <div className="flex flex-col items-center gap-4 text-center">
 <div className="w-16 h-16 rounded-full bg-bg-tertiary flex items-center justify-center border border-border group-hover:scale-110 transition-transform duration-500 ">
 <Upload size={28} className="text-text-muted group-hover:text-primary transition-colors" />
 </div>
 <span className="text-xs font-semibold uppercase tracking-widest text-text-muted group-hover:text-primary transition-colors">Transmissão de Edital / Proposta</span>
 </div>
 </div>
 <p className="text-xs text-center text-text-muted italic font-semibold uppercase tracking-widest opacity-40">Nenhuma evidência anexada ao dossiê.</p>
 </div>
 </div>
 ...
 <div className="card p-8 bg-gradient-primary border border-primary/20 flex flex-col items-center gap-6 group">
 ...
 <h4 className="text-white font-semibold uppercase text-xs">Neural Win-Score</h4>
 <p className="text-4xl font-semibold text-white tracking-tight group-hover:scale-110 transition-transform duration-700">84%</p>
 </div>
 <p className="text-xs text-white/60 font-semibold uppercase tracking-widest text-center leading-relaxed">Alta probabilidade de êxito baseada no histórico de lances e precificação.</p>
 </div>
 </div>
 </div>
 </div>
