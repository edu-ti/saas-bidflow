...
.catch(() => setStages(defaultStages)); // fallback to defaults
 ...
 <span className="text-xs font-bold text-slate-600 ml-1">{Math.round(parseFloat(opp.win_probability))}%</span>
 </div>
 )}
 ...
 <span className="flex items-center text-xs bg-red-100 text-red-700 px-1.5 py-0.5 rounded font-semibold">
 <FileWarning size={10} className="mr-1" /> Alto Risco
 </span>
 )}
 {opp.bidding_metadata.data_limite_impugnacao && (
 <span className="flex items-center text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-semibold">
 ...
 <span className="flex items-center text-xs bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded font-semibold">
