...
export default function Tasks() {
 ...
 Task <span className="text-primary">Orchestration</span>
 </h1>
 ...
 <div key={i} className="card p-8 flex items-center justify-between group bg-bg-tertiary/10 border-border/30 overflow-hidden relative">
 <div className="space-y-2 relative z-10">
 <p className="text-xs font-semibold uppercase text-text-muted opacity-60">{stat.label}</p>
 <p className="text-2xl font-semibold text-text-primary tracking-tight group-hover:text-primary transition-colors duration-500">{stat.val}</p>
 </div>
 <div className={`p-5 rounded-2xl ${stat.bg} border border-border ${stat.color} group-hover:scale-110 transition-transform duration-500 relative z-10`}>
 <stat.icon size={28} />
 </div>
 ...
 <div className="card p-8 flex flex-col lg:flex-row items-center justify-between gap-8 bg-bg-tertiary/10 backdrop-blur-xl shrink-0">
 ...
 className={`px-8 py-3 rounded-xl text-xs font-semibold uppercase tracking-wider transition-all duration-300 ${
 filter === f.v ? 'bg-primary text-background ' : 'text-text-muted hover:text-text-primary hover:bg-bg-tertiary/40'
 }`}
 >
 {f.l}
 </button>
 ...
 <p className="text-xs font-semibold uppercase ">Sincronizando Pipeline Global...</p>
 </div>
 ) : tasks.length === 0 ? (
 <div className="py-32 text-center card border-dashed bg-bg-tertiary/5">
 <Target size={56} className="mx-auto text-primary/20 mb-6" />
 <p className="text-xs font-semibold uppercase text-text-muted">Nenhuma diretriz localizada no radar estratégico</p>
 </div>
 ) : (
 tasks.map(task => (
 <div
 ...
 : 'bg-bg-tertiary border-border text-transparent hover:border-primary/50 hover:text-primary/50'
 }`}
 >
 ...
 <h3 className={`font-semibold text-text-primary text-lg tracking-tight uppercase transition-all duration-500 ${task.status === 'completed' ? 'line-through text-text-muted' : 'group-hover:text-primary'}`}>
 {task.title}
 </h3>
 <span className={`text-xs font-semibold uppercase tracking-wider px-3 py-1.5 rounded-xl border backdrop-blur-md ${
 ...
 <div className="flex items-center gap-3 text-xs font-semibold uppercase tracking-widest text-text-muted opacity-40">
 <Clock size={14} />
 ...
 <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editingTask ? 'REFINAR DIRETRIZ ' : 'NOVA MISSÃO ESTRATÉGICA'} size="md">
 <form onSubmit={handleSave} className="space-y-10 p-2">
 <div className="space-y-3">
 <label className="text-xs font-semibold text-text-muted uppercase tracking-widest px-1">Título da Missão *</label>
 <input
 type="text"
 required
 value={formData.title}
 onChange={(e) => setFormData({...formData, title: e.target.value })}
 className="w-full px-6 py-4 bg-background border border-border rounded-2xl text-sm font-bold text-text-primary focus:border-primary/40 outline-none transition-all "
 placeholder="Ex: Revisar documentação do Pregão 45/2024"
 />
 </div>

 <div className="space-y-3">
 <label className="text-xs font-semibold text-text-muted uppercase tracking-widest px-1">Resumo Executivo / Briefing</label>
 <textarea
 value={formData.description}
 onChange={(e) => setFormData({...formData, description: e.target.value })}
 className="w-full px-6 py-4 bg-background border border-border rounded-2xl text-sm font-medium text-text-primary focus:border-primary/40 outline-none resize-none transition-all "
 rows={3}
 ...
 <div className="space-y-3">
 <label className="text-xs font-semibold text-text-muted uppercase tracking-widest px-1">Prazo Fatal (Deadline)</label>
 <DatePicker
 ...
 <div className="space-y-3">
 <label className="text-xs font-semibold text-text-muted uppercase tracking-widest px-1">Nível de Prioridade</label>
 <Select
 ...
 <div className="space-y-3">
 <label className="text-xs font-semibold text-text-muted uppercase tracking-widest px-1">Agente Responsável / Assignee</label>
 ...
 className="w-full pl-16 pr-6 py-4 bg-background border border-border rounded-2xl text-sm font-bold text-text-primary focus:border-primary/40 outline-none transition-all "
 ...
 className="px-8 py-4 text-text-muted font-semibold hover:text-text-primary transition-all text-xs uppercase tracking-widest"
 >
 Descartar
 </button>
 <button
 type="submit"
 className="btn btn-primary py-4 px-12 flex items-center gap-3 uppercase text-xs tracking-widest"
 >
 <Zap size={20} />
