...
export default function TenantList() {
 ...
 Tenant <span className="text-primary">Governance</span>
 </h1>
 <p className="text-text-secondary max-w-prose-ui flex items-center gap-2 text-sm font-medium">
 <Globe size={14} className="text-primary" />
 Gestão de infraestrutura multi-tenant e provisionamento de ecossistemas.
 </p>
 </div>
 ...
 className="pl-12 pr-6 py-4 w-full lg:w-80 rounded-2xl border border-border bg-bg-tertiary/10 text-text-primary focus:border-primary/40 outline-none transition-all font-semibold text-xs uppercase tracking-widest placeholder:text-text-secondary/60"
 />
 </div>
 <button
 onClick={() => openModal()}
 className="btn btn-primary py-4 px-10 flex items-center gap-4 uppercase text-xs tracking-widest whitespace-nowrap group"
 >
 ...
 <div className="px-10 py-6 bg-bg-tertiary/20 border-b border-border/30 flex items-center justify-between">
 <div className="flex items-center gap-4">
 <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary ">
 <ShieldCheck size={20} />
 </div>
 <h3 className="text-xs font-semibold text-text-primary uppercase ">Clusters Corporativos Ativos</h3>
 </div>
 ...
 <tr>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-secondary">Instância / Branding</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-secondary">Matriz Documental</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-secondary">Power Tier</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-secondary text-center">Nodes</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-secondary">Estado</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-secondary text-right">Ações de Tunelamento</th>
 </tr>
 </thead>
 ...
 <p className="text-xs font-semibold uppercase text-text-secondary animate-pulse">Sincronizando Malha Neural Multi-Tenant...</p>
 </div>
 </td>
 </tr>
 ...
 <p className="text-sm font-semibold uppercase text-text-secondary">Nenhuma instância detectada na varredura global.</p>
 </div>
 </td>
 </tr>
 ) : (
 filteredTenants.map((tenant) => (
 <tr key={tenant.id} className="hover:bg-bg-tertiary/30 transition-all group border-b border-border/10 duration-500">
 <td className="px-10 py-10">
 <div className="flex items-center gap-6">
 <div className="w-14 h-14 rounded-2xl bg-bg-tertiary border border-border/30 flex items-center justify-center group-hover:scale-110 transition-transform duration-700 relative overflow-hidden">
 <Building2 className="w-7 h-7 text-primary relative z-10" />
 <div className="absolute inset-0 bg-primary/5 opacity-0 group-hover:opacity-100 transition-opacity" />
 </div>
 <div>
 <p className="font-semibold text-text-primary uppercase tracking-tight text-base group-hover:text-primary transition-colors">{tenant.name}</p>
 <div className="flex items-center gap-3 mt-1">
 <span className="text-xs text-text-secondary font-semibold uppercase tracking-widest">ID: {tenant.id.toString().padStart(4, '0')}</span>
 <div className="w-1 h-1 rounded-full bg-border-subtle" />
 <span className="text-xs text-text-secondary font-semibold uppercase tracking-widest italic">{new Date(tenant.created_at).toLocaleDateString('pt-BR')}</span>
 </div>
 </div>
 </div>
 </td>
 <td className="px-10 py-10">
 <div className="text-sm text-text-secondary font-semibold tracking-wider bg-bg-tertiary/40 px-4 py-2 rounded-xl border border-border inline-block uppercase">
 {tenant.cnpj || 'DRAFT_CNPJ'}
 </div>
 </td>
 <td className="px-10 py-10">
 <span className="inline-flex items-center gap-3 px-5 py-2 rounded-2xl bg-primary/10 text-primary border border-primary/20 text-xs font-semibold uppercase tracking-widest ">
 ...
 <span className="text-lg font-semibold text-text-primary tracking-tight">{tenant.users_count}</span>
 <div className="w-12 h-1 bg-bg-tertiary/60 rounded-full overflow-hidden border border-border/20">
 <div className="h-full bg-primary" style={{ width: `${Math.min((tenant.users_count / (tenant.max_users || 100)) * 100, 100)}%` }} />
 </div>
 <span className="text-xs font-semibold text-text-secondary uppercase tracking-widest">Limit: {tenant.max_users || '∞'}</span>
 </div>
 </td>
 <td className="px-10 py-10">
 <span className={`inline-flex items-center gap-3 px-4 py-2 rounded-2xl text-xs font-semibold uppercase tracking-widest border backdrop-blur-md ${
 ...
 <div className={`w-2 h-2 rounded-full bg-current ${tenant.status === 'active' ? 'animate-pulse ' : ''}`}></div>
 ...
 className="btn btn-primary py-3.5 px-8 text-xs font-semibold tracking-wider flex items-center gap-4 group/btn"
 >
 ...
 <h3 className="text-xs font-semibold text-text-primary uppercase ">Identidade Neural do Super Admin</h3>
 </div>
 
 <div className="group">
 <label className="text-xs font-semibold text-text-secondary uppercase px-4 group-focus-within:text-primary transition-colors">Nome Completo do Gestor de Cluster</label>
 <div className="relative mt-3">
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
 <div className="group">
 <label className="text-xs font-semibold text-text-secondary uppercase px-4 group-focus-within:text-primary transition-colors">E-mail Primário de Túnel</label>
 <div className="relative mt-3">
 ...
 className="w-full bg-background/50 border border-border rounded-xl pl-20 pr-8 py-6 text-sm font-semibold text-text-primary outline-none focus:border-primary/40 transition-all font-mono lowercase tracking-wider"
 ...
 <div className="group">
 <label className="text-xs font-semibold text-text-secondary uppercase px-4 group-focus-within:text-primary transition-colors">
 ...
 className="w-full bg-background/50 border border-border rounded-xl pl-20 pr-8 py-6 text-sm font-semibold text-text-primary outline-none focus:border-primary/40 transition-all "
 />
 </div>
 </div>
 </div>
