...
...node,
 ...
 {/* Header */}
 <header className="p-6 border-b border-border flex justify-between items-center bg-bg-secondary/30 backdrop-blur-md">
 ...
 Chatbot <span className="text-primary">Architect</span>
 </h1>
 <p className="text-xs font-semibold text-text-muted uppercase tracking-widest">IA Conversacional Neural</p>
 </div>
 </div>
 ...
 onClick={handleSave} 
 disabled={isSaving}
 className="btn btn-primary px-8 py-2.5 rounded-xl text-xs font-semibold uppercase tracking-widest flex items-center gap-3 "
 >
 ...
 <aside className="w-72 border-r border-border bg-bg-secondary/20 p-6 flex flex-col gap-8">
 ...
 <div
 key={item.type}
 className="card p-4 flex items-center gap-4 cursor-grab active:cursor-grabbing hover:border-primary/40 transition-all group bg-bg-tertiary/5"
 onDragStart={(event) => {
 ...
 <span className="text-xs font-semibold uppercase tracking-widest text-text-primary">{item.label}</span>
 </div>
 ))}
 </div>
 </div>

 <div className="mt-auto card p-5 bg-primary/5 border-primary/20 space-y-3">
 <div className="flex items-center gap-2 text-primary">
 <Settings size={14} className="animate-spin-slow" />
 <span className="text-xs font-semibold uppercase tracking-widest">Guia Rápido</span>
 </div>
 <p className="text-xs text-text-secondary leading-relaxed font-bold opacity-70">
 ...
 <Controls className="!bg-bg-secondary !border-border ! fill-text-primary" />
 <Panel position="top-right" className="bg-bg-secondary/80 backdrop-blur-md p-3 rounded-xl border border-border ">
 ...
 {selectedNode && (
 <aside className="w-80 border-l border-border bg-bg-secondary/30 backdrop-blur-xl p-8 animate-in slide-in-from-right-4 duration-500 overflow-y-auto">
 <div className="flex items-center justify-between mb-10">
 <h3 className="text-xs font-semibold uppercase text-text-primary">Propriedades</h3>
 <button onClick={() => setSelectedNode(null)} className="text-text-muted hover:text-text-primary"><X size={18} /></button>
 </div>

 <div className="space-y-8">
 <div className="space-y-3">
 <label className="text-xs font-semibold uppercase tracking-widest text-text-muted">Conteúdo do Bloco</label>
 <textarea
 className="w-full bg-background/50 border border-border rounded-xl p-4 text-sm font-bold text-text-primary outline-none focus:border-primary/40 transition-all h-32"
 ...
 <div className="space-y-4">
 <label className="text-xs font-semibold uppercase tracking-widest text-text-muted">Opções de Resposta</label>
 <div className="space-y-2">
 {(selectedNode.data.options || []).map((opt: string, i: number) => (
 <div key={i} className="flex gap-2">
 <input
 className="flex-1 bg-background/50 border border-border rounded-xl px-4 py-2 text-xs font-bold"
 value={opt}
 onChange={(e) => {
 ...
 className="w-full py-2 bg-primary/10 border border-primary/20 rounded-xl text-primary text-xs font-semibold uppercase tracking-widest hover:bg-primary/20 transition-all flex items-center justify-center gap-2"
 >
 ...
 <div className="space-y-3">
 <label className="text-xs font-semibold uppercase tracking-widest text-text-muted">Tipo de Dado</label>
 <select
 className="w-full bg-background/50 border border-border rounded-xl px-4 py-3 text-xs font-bold text-text-primary outline-none"
 ...
 <button 
 onClick={deleteNode}
 className="w-full py-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-500 text-xs font-semibold uppercase tracking-widest hover:bg-red-500/20 transition-all flex items-center justify-center gap-3"
 >
