...
import {
  ...
  AlertTriangle, DollarSign, Boxes, X, Filter, Warehouse, ShieldCheck, Zap, Globe, Activity, Database, ChevronRight, Layout, BarChart3, Target, Loader2, Check, Settings
} from 'lucide-react';
...
...formData,
 ...
 Asset & <span className="text-primary">Inventory Control</span>
 </h1>
 <p className="text-text-secondary max-w-prose-ui flex items-center gap-2 text-sm font-medium">
 <Warehouse size={14} className="text-primary" />
 Gestão estratégica de ativos, valuation e logística global.
 </p>
 </div>
 <div className="flex items-center gap-5 bg-bg-secondary/20 border border-border/30 p-5 rounded-2xl ">
 <div className="flex flex-col items-end gap-1">
 <span className="text-xs font-semibold uppercase tracking-widest text-text-muted italic opacity-60">Saúde Logística</span>
 <span className="text-xs font-semibold text-emerald-500 uppercase tracking-widest bg-emerald-500/10 px-2 py-0.5 rounded-md">Operação Estável</span>
 </div>
 ...
 {kpis.map((kpi, i) => (
 <div key={i} className="card p-8 flex flex-col gap-8 group hover:border-primary/40 transition-all duration-500 bg-bg-secondary/10 backdrop-blur-xl border-border/30 ">
 ...
 <kpi.icon className={`w-7 h-7 ${kpi.color} group-hover: transition-all`} />
 </div>
 <div>
 <p className="text-xs font-semibold uppercase text-text-muted opacity-60">{kpi.label}</p>
 <p className="text-2xl font-semibold text-text-primary mt-3 tracking-tight group-hover:text-primary transition-colors duration-500">{kpi.val}</p>
 </div>
 </div>
 ))}
 </div>
 ...
 {/* Tabs */}
 <div className="flex items-center gap-4 bg-bg-secondary/20 border border-border/30 p-2 rounded-xl w-fit backdrop-blur-md">
 {[
 ...
 className={`flex items-center gap-3 px-8 py-3 rounded-[2.2rem] text-xs font-semibold uppercase tracking-wider transition-all duration-500 ${
 activeTab === tab.key 
 ...
 <div className="p-8 bg-bg-secondary/20 border-b border-border/30 flex flex-wrap gap-8 items-center justify-between">
 ...
 <tr>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60">SKU / ID</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60">Descrição do Ativo</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60">Marca / Categ.</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60 text-center">Físico</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60 text-center">Disponível</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60 text-right">Custo</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60 text-right">Venda</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60 text-right">Ações</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-border/20">
 {loading ? (
 <tr><td colSpan={8} className="px-10 py-40 text-center text-text-muted uppercase text-xs font-semibold tracking-widest animate-pulse"><Loader2 className="animate-spin inline mr-6 w-12 h-12 text-primary" /> Sincronizando Matriz...</td></tr>
 ) : products.length === 0 ? (
 <tr><td colSpan={8} className="px-10 py-40 text-center text-text-muted uppercase text-xs font-semibold tracking-widest opacity-40">Nenhum ativo localizado na base estratégica</td></tr>
 ) : (
 products.map(product => {
 const available = (product.on_hand_qty || 0) - (product.reserved_qty || 0);
 return (
 <tr key={product.id} className="hover:bg-bg-secondary/20 transition-all group border-b border-border/10 duration-500">
 <td className="px-10 py-10 font-mono text-sm text-primary font-semibold uppercase tracking-widest">
 <span className="bg-primary/5 px-2 py-1 rounded border border-primary/20 ">{product.sku || '---'}</span>
 </td>
 <td className="px-10 py-10">
 <div className="font-semibold text-text-primary group-hover:text-primary transition-colors uppercase tracking-tight text-sm">{product.product?.name || 'Item não identificado'}</div>
 <div className="text-xs text-text-muted font-semibold uppercase tracking-widest mt-2.5 opacity-60 flex items-center gap-2">
 <Warehouse size={12} className="text-primary/40" /> {product.depot?.name || 'Repositório Central'}
 </div>
 </td>
 <td className="px-10 py-10">
 <div className="text-xs font-semibold text-text-primary uppercase tracking-wider">{product.brand?.name || 'Genérica'}</div>
 <div className="text-xs text-text-muted font-semibold uppercase tracking-widest mt-1 opacity-50">{product.category?.name || 'Miscelânea'}</div>
 </td>
 <td className="px-10 py-10 text-center font-semibold text-text-primary text-base tracking-tight">{product.on_hand_qty || 0}</td>
 <td className="px-10 py-10 text-center">
 <span className={`px-4 py-2 rounded-xl text-xs font-semibold uppercase tracking-wider border backdrop-blur-md ${
 ...
 {activeTab === 'movements' && (
 <div className="card overflow-hidden bg-bg-secondary/10 backdrop-blur-md border-border/30 ">
 <div className="p-10 border-b border-border/30 bg-bg-secondary/20 flex justify-between items-center">
 <h3 className="text-sm font-semibold text-text-primary uppercase flex items-center gap-4">
 <div className="w-1.5 h-6 bg-primary rounded-full " />
 Trilha de Auditoria Logística Core
 </h3>
 <div className="px-5 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full text-xs font-semibold text-emerald-500 uppercase tracking-widest animate-pulse">
 ...
 <thead className="bg-bg-secondary/40 border-b border-border">
 <tr>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60">Operação</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60">Cronologia</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60">Categoria</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60">Origem / Destino</th>
 <th className="px-10 py-6 font-semibold uppercase text-xs text-text-muted opacity-60 text-right">Valuation Operacional</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-border/20">
 {movements.length === 0 ? (
 <tr><td colSpan={5} className="px-10 py-40 text-center text-text-muted uppercase text-xs font-semibold tracking-widest opacity-40">Nenhuma movimentação auditada no ledger</td></tr>
 ) : (
 movements.map(m => (
 <tr key={m.id} className="hover:bg-bg-secondary/20 transition-all group border-b border-border/10 duration-500">
 <td className="px-10 py-10">
 <span className={`px-4 py-2 rounded-xl text-xs font-semibold uppercase tracking-wider flex items-center gap-3 w-fit border backdrop-blur-md ${
 ...
 <td className="px-10 py-10 text-text-muted font-semibold text-xs uppercase tracking-widest opacity-60">{new Date(m.date).toLocaleString('pt-BR')}</td>
 <td className="px-10 py-10 text-text-primary font-semibold uppercase tracking-tight text-xs">{m.categoryName || 'Fluxo Geral'}</td>
 <td className="px-10 py-10 text-text-secondary font-bold text-xs uppercase tracking-tight opacity-80">{m.entity || 'Core System'}</td>
 <td className="px-10 py-10 text-right text-text-primary font-semibold text-sm tracking-tight group-hover:text-primary transition-colors">{formatCurrency(m.totalValue)}</td>
 </tr>
 ))
 )}
 </tbody>
 ...
 <div key={i} className="card p-10 flex flex-col gap-10 group hover:border-primary/40 transition-all duration-700 bg-bg-secondary/10 backdrop-blur-md border-border/30 ">
 ...
 <p className="text-2xl font-semibold text-text-primary tracking-tight group-hover:text-primary transition-colors duration-700">{item.count}</p>
 <span className="text-xs text-text-muted font-semibold uppercase tracking-widest opacity-40">Entidades Ativas</span>
 </div>
 </div>
 </div>
 ))}
 ...
 {editingProduct ? 'REFINAR' : 'NOVO'} <span className="text-primary">ATIVO DE ESTOQUE</span>
 </h2>
 ...
 <button onClick={() => setShowModal(false)} className="p-4 bg-bg-secondary/40 border border-border rounded-2xl text-text-muted hover:text-text-primary transition-all ">
 <X size={24} />
 </button>
 </div>

 <form onSubmit={handleSave} className="p-10 overflow-y-auto space-y-12">
 <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
 <div className="space-y-4 group">
 <label className="text-xs font-semibold text-text-muted uppercase px-2 group-focus-within:text-primary transition-colors">SKU / Identificador Único Neural</label>
 ...
 <div className="space-y-4 group">
 <label className="text-xs font-semibold text-text-muted uppercase px-2 group-focus-within:text-primary transition-colors">Código de Barras Global (EAN)</label>
 <div className="relative">
 ...
 <label className="text-xs font-semibold text-text-muted uppercase px-2 group-focus-within:text-primary transition-colors">Depósito Alvo Logístico</label>
 <div className="relative">
...
...depots.map(d => ({ value: d.id.toString(), label: d.name.toUpperCase() }))
 ...
 <label className="text-xs font-semibold text-text-muted uppercase px-2 group-focus-within:text-primary transition-colors">Marca Estratégica </label>
 <div className="relative">
...
...brands.map(b => ({ value: b.id.toString(), label: b.name.toUpperCase() }))
 ...
 <label className="text-xs font-semibold text-text-muted uppercase px-2 group-focus-within:text-primary transition-colors">Categoria Operacional Core</label>
 <div className="relative">
...
...categories.map(c => ({ value: c.id.toString(), label: c.name.toUpperCase() }))
 ...
 <div className="space-y-10 pt-10 border-t border-border/30">
 <h3 className="text-sm font-semibold text-primary uppercase flex items-center gap-4"><div className="w-6 h-px bg-primary/30" /> Engenharia de Preços & Valuation <div className="w-6 h-px bg-primary/30" /></h3>
 <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
 <div className="space-y-4 group">
 <label className="text-xs font-semibold text-text-muted uppercase px-2 group-focus-within:text-primary transition-colors">Preço Custo (BRL)</label>
 <div className="relative">
 ...
 <div className="space-y-4 group">
 <label className="text-xs font-semibold text-text-muted uppercase px-2 group-focus-within:text-primary transition-colors">Markup Operacional (%)</label>
 <div className="relative">
 ...
 <div className="space-y-4 group">
 <label className="text-xs font-semibold text-text-muted uppercase px-2">Valuation de Venda Final</label>
 <div className="w-full bg-primary/5 border border-primary/20 rounded-2xl px-8 py-5 text-base font-semibold text-primary tracking-tight flex items-center justify-between">
 <span>BRL</span>
 ...
 <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
 <div className="space-y-4 group">
 <label className="text-xs font-semibold text-text-muted uppercase px-2 group-focus-within:text-primary transition-colors">Volume Físico On-Hand</label>
 <div className="relative">
 ...
 <div className="space-y-4 group">
 <label className="text-xs font-semibold text-text-muted uppercase px-2 group-focus-within:text-primary transition-colors">Segurança Mínima (SLA)</label>
 <div className="relative">
 ...
 <div className="space-y-4 group">
 <label className="text-xs font-semibold text-text-muted uppercase px-2 group-focus-within:text-primary transition-colors">Capacidade Teto do Hub</label>
 <div className="relative">
 ...
 <div className="flex justify-end gap-6 pt-12 border-t border-border/30">
 <button
 type="button"
 ...
 type="submit"
 disabled={loading}
 className="btn btn-primary py-5 px-16 flex items-center gap-5 uppercase text-[12px] disabled:opacity-60"
 >
 {loading ? <Loader2 size={24} className="animate-spin" /> : <ShieldCheck size={24} className="" />}
 ...
 <div className="bg-bg-secondary border border-border/30 rounded-xl w-full max-w-lg overflow-hidden flex flex-col scale-100 animate-in zoom-in-95 duration-500">
 <div className="flex justify-between items-center px-10 py-8 bg-bg-secondary/20 border-b border-border/30">
 <h2 className="text-xl font-semibold text-text-primary uppercase tracking-tight flex items-center gap-4">
 NOVO <span className="text-primary uppercase">{getSettingsLabel()}</span>
 </h2>
 <button onClick={() => setShowSettingsModal(false)} className="p-4 bg-bg-secondary/40 border border-border rounded-2xl text-text-muted hover:text-text-primary transition-all ">
 <X size={24} />
 </button>
 </div>
 ...
 <div className="space-y-4 group">
 <label className="text-xs font-semibold text-text-muted uppercase px-2 group-focus-within:text-primary transition-colors">Identificação Estratégica *</label>
 <div className="relative">
 ...
 <div className="space-y-4 group">
 <label className="text-xs font-semibold text-text-muted uppercase px-2 group-focus-within:text-primary transition-colors">Abreviação Digital (Acro) *</label>
 <div className="relative">
 ...
 className="px-8 py-4 text-sm font-semibold text-text-muted hover:text-text-primary uppercase transition-all"
 >
 CANCELAR
 </button>
 <button
 type="submit"
 disabled={loading}
 className="btn btn-primary py-4 px-10 uppercase text-sm flex items-center gap-4"
 >
